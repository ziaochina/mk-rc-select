...
function nodeToJson(node) {
  ...
  if (!(0, _reactCompat.isElement)(node)) {
    ...
    } else if (node._reactInternalInstance /* && node._reactInternalInstance.constructor && node._reactInternalInstance.constructor.name === "ShallowComponentWrapper" */
    ) {
...
exports.default = function (wrapper) {
  if (wrapper.length > 1) {
    var nodes = wrapper.getNodes ? wrapper.getNodes() : wrapper.nodes;
    return nodes.map(nodeToJson);
  }

  var node = wrapper.getNode ? wrapper.getNode() : wrapper.node;
